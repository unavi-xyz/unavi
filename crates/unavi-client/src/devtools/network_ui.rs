...
pub fn update_network_stats_text(
    ...
    if !stats.is_changed() && !stats.peers.is_empty() {
        return;
    }
    ...
    // Add new stat text for each peer.
    for (peer_id, peer_stats) in &stats.peers {
        ...
        let quality_str = match peer_stats.quality_score {
        ...
        let quality_color = match peer_stats.quality_score {
        ...
        commands.entity(overlay_entity).with_children(|parent| {
            parent
                .spawn((
                    ...
                    NetworkStatsText { peer_id: *peer_id },
                ))
                .with_children(|parent| {
