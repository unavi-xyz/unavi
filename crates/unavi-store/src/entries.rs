...
pub async fn get(doc: &Doc, key: &str) -> anyhow::Result<Option<DocEntry>> {
    let query = Query::single_latest_per_key().key_exact(key);
    Ok(doc.get_one(query).await?.map(|entry| DocEntry {
...
pub async fn fetch(
    ...
    let doc = crate::namespace::ensure_open(docs, ns).await?;
    if !peers.is_empty() {
