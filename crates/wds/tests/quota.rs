...
async fn test_quota_shared_by_multiple_pinners(#[future] ctx: DataStoreCtx) {
    ...
    // Get initial quotas.
    let alice_initial: i64 = sqlx::query_scalar!(
        "SELECT bytes_used FROM user_quotas WHERE owner = ?",
        alice_did
    )
    .fetch_one(ctx.store.db())
    ...
    let bob_initial: i64 = sqlx::query_scalar!(
        "SELECT bytes_used FROM user_quotas WHERE owner = ?",
        bob_did
    )
    .fetch_one(ctx.store.db())
    ...
    let alice_after: i64 = sqlx::query_scalar!(
        "SELECT bytes_used FROM user_quotas WHERE owner = ?",
        alice_did
    )
    .fetch_one(ctx.store.db())
    ...
    let bob_after: i64 = sqlx::query_scalar!(
        "SELECT bytes_used FROM user_quotas WHERE owner = ?",
        bob_did
    )
    .fetch_one(ctx.store.db())
    ...
    let alice_gc: i64 = sqlx::query_scalar!(
        "SELECT bytes_used FROM user_quotas WHERE owner = ?",
        alice_did
    )
    .fetch_one(ctx.store.db())
