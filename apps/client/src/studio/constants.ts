...
import { DEFAULT_TRANSFORM, IEntity, IScene } from "@wired-xr/engine";
...
const box: IEntity<"Box"> = deepClone(ALL_PRESETS["Box"]);
box.id = nanoid();
box.parentId = "root";
box.props.width = 10;
...
const spawn: IEntity<"Spawn"> = deepClone(ALL_PRESETS["Spawn"]);
spawn.id = nanoid();
spawn.parentId = "root";
spawn.transform.position = [0, 2, 0];

export const STARTING_SCENE: IScene = {
  tree: {
    type: "Group",
