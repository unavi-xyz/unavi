...
use common::{DID_ALICE, create_test_store};
...
async fn test_empty_blob() {
    ...
    let empty: &[u8] = b"";
    let blob_id = store.store_blob(empty).await.expect("store empty blob");

    let retrieved = store.get_blob(&blob_id).expect("get blob").expect("exists");
...
async fn test_binary_blob_content() {
    ...
    let blob_id = store.store_blob(&binary_data).await.expect("store blob");
...
async fn test_duplicate_blob_storage() {
    ...
    let id1 = store.store_blob(data).await.expect("store first");
    let id2 = store.store_blob(data).await.expect("store second");

    // Should return same CID (content-addressed).
    assert_eq!(id1, id2);

    // Should still be retrievable.
    let retrieved = store.get_blob(&id1).expect("get blob").expect("exists");
...
async fn test_pin_then_unpin_then_gc() {
    ...
    let id = store.create_record(genesis).await.expect("create");

    store.pin_record(&id, None).await.expect("pin");
    store.unpin_record(&id).await.expect("unpin");
