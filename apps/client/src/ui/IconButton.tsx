  ...
  ({ selected, disabled, rounded = "small", cursor = "default", children, ...rest }, ref) => {
    ...
    return (
      <button
        ref={ref}
        disabled={disabled}
        className={`flex aspect-square h-full items-center justify-center text-2xl transition active:opacity-80 ${cursorClass} ${roundedClass} ${
          disabled ? "cursor-not-allowed" : selectedClass
        }`}
        {...rest}
      >
        {children}
