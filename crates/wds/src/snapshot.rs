//! Snapshot-doc persistence primitive.
//!
//! A snapshot doc is an iroh-docs namespace whose `snapshot` key points at an
//! opaque payload blob, with one `deps/<hash>` entry per transitive blob
//! dependency. Nothing reads the `deps/` prefix back: recording each dependency
//! as an entry is what makes a syncing host fetch, tag and meter the blob,
//! without interpreting content. This module deals only in bytes, hashes and
//! namespaces; the payload encoding is the caller's concern.
...
fn dep_key(hash: Hash) -> String {
    format!("{DEPS_PREFIX}{}", hash.to_hex())
}

/// Creates a new namespace holding a `snapshot` blob plus `deps/` entries for
/// its transitive blob dependencies, returning the namespace id.
pub async fn create_doc(
    ...
    let ns = doc.id();
    write(&doc, blobs, author, snapshot, deps).await?;
    Ok(ns)
}
...
pub async fn write(
    ...
    doc.set_hash(author, SNAPSHOT, info.hash, len).await?;
    for (hash, size) in deps {
        doc.set_hash(author, dep_key(*hash), *hash, *size).await?;
    }
    Ok(())
}
...
pub async fn fetch(
    ...
    for _ in 0..attempts.max(1) {
        let query = Query::single_latest_per_key().key_exact(SNAPSHOT);
...
pub async fn read(docs: &Docs, blobs: &Blobs, ns: NamespaceId) -> anyhow::Result<Option<Bytes>> {
