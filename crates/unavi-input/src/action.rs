...
impl Action {
    pub const ALL: [Self; Self::COUNT] = [
        ...
        Self::Jump,
        Self::Sprint,
        Self::Grab(PointerKind::Screen),
        Self::Grab(PointerKind::LeftHand),
        Self::Grab(PointerKind::RightHand),
        Self::Menu(PointerKind::Screen),
        Self::Menu(PointerKind::LeftHand),
        Self::Menu(PointerKind::RightHand),
    ];
    pub const COUNT: usize = 4 + 2 * PointerKind::COUNT;
    ...
    const fn index(self) -> usize {
        match self {
            ...
            Self::Jump => 2,
            Self::Sprint => 3,
            Self::Grab(kind) => 4 + kind.index(),
            Self::Menu(kind) => 4 + PointerKind::COUNT + kind.index(),
        }
    }
...
impl ActionState {
    ...
    pub fn end_frame(&mut self, tuning: &Tuning) {
        ...
        let movement = &mut self.values[Action::Move.index()].axis;
...
pub fn end_frame(mut state: ResMut<ActionState>, config: Res<InputConfig>) {
...
mod tests {
    ...
    fn the_hardest_pull_wins_rather_than_summing() {
        ...
        state.press(Action::Grab(PointerKind::Screen), 0.8);
        state.press(Action::Grab(PointerKind::Screen), 0.3);
        assert!((state.value(Action::Grab(PointerKind::Screen)) - 0.8).abs() < f32::EPSILON);
    }
    ...
    fn a_squeeze_short_of_the_threshold_is_not_a_press() {
        let mut state = ActionState::default();
        let grab = Action::Grab(PointerKind::RightHand);
        let tuning = Tuning::default();

        state.begin_frame();
        state.press(grab, tuning.press_threshold - 0.01);
        state.end_frame(&tuning);
        assert!(!state.pressed(grab), "but its pull is still readable");
        assert!(state.value(grab) > 0.0);
    }
