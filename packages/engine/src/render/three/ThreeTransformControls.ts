...
const _mouseUpEvent = { mode: null, type: "mouseUp" };
const _objectChangeEvent = { type: "objectChange" };

class ThreeTransformControls extends Object3D {
...
function onPointerHover(event: FakePointerEvent) {
  ...
  switch (event.detail.pointerType) {
    case "mouse":
    case "pen":
      this.pointerHover(event.detail.pointer);
      break;
  }
}
...
function onPointerUp(event: FakePointerEvent) {
  ...
  this.pointerUp(event.detail.pointer);
}
...
export { ThreeTransformControls, TransformControlsGizmo, TransformControlsPlane };
