...
export function useSave() {
  ...
  async function save() {
    ...
    // Upload files to S3
    scene.entities.forEach((node) => {
      // glTF models
      if (node.mesh?.type === "glTF") {
        const uri = node.mesh.uri;
        if (uri) promises.push(uploadFile(uri, node.id, "model"));
      }

      // Images
      if (node.materialId) {
        const material = engine.scene.materials[node.materialId];
