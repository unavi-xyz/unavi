...
async fn test_default_quota_is_5gb() {
    ...
    let quota = view.get_storage_quota().await.expect("get quota");
...
async fn test_blob_upload_consumes_quota() {
    ...
    view.store_blob(BLOB_HELLO).await.expect("store blob");
...
async fn test_same_blob_uploaded_twice_charges_once() {
    ...
    view.store_blob(BLOB_HELLO)
        .await
        .expect("store blob first time");
    let after_first = view.get_storage_quota().await.expect("get quota");

    view.store_blob(BLOB_HELLO)
        .await
        .expect("store blob second time");
    let after_second = view.get_storage_quota().await.expect("get quota");
...
async fn test_custom_quota_setting() {
    ...
    view.set_storage_quota(custom_quota)
        .await
        .expect("set quota");
...
async fn test_gc_releases_blob_quota() {
    ...
    let before_gc = view.get_storage_quota().await.expect("get quota");
...
async fn test_quota_isolation_between_users() {
    ...
    let alice_quota = view_alice.get_storage_quota().await.expect("alice quota");
    let bob_quota = view_bob.get_storage_quota().await.expect("bob quota");
...
async fn test_bytes_remaining() {
    ...
    view.set_storage_quota(100).await.expect("set quota");
    view.store_blob(b"test").await.expect("store blob");
