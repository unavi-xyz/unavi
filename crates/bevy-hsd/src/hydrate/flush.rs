...
pub(crate) fn flush_script_dirty(
    ...
    for (doc_ent, registry, queue) in &docs {
        flush_queued_events(doc_ent, queue, &mut commands);
        flush_dirty_nodes(doc_ent, registry, &mut commands);
        flush_dirty_meshes(doc_ent, registry, &mut commands);
        flush_dirty_materials(doc_ent, registry, &mut commands);
    }
}

fn flush_queued_events(doc_ent: Entity, queue: &ScriptEventQueue, commands: &mut Commands) {
    let events: Vec<ScriptQueuedEvent> = queue
        .0
        .lock()
...
fn flush_dirty_nodes(doc_ent: Entity, registry: &SceneRegistry, commands: &mut Commands) {
...
fn flush_dirty_meshes(doc_ent: Entity, registry: &SceneRegistry, commands: &mut Commands) {
...
fn flush_dirty_materials(doc_ent: Entity, registry: &SceneRegistry, commands: &mut Commands) {
